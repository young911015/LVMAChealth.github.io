...
(function(JustGage) {
  ...
  Gauges.initGauge = function(gaugeNode, reset) {
    ...
    if(!!dataEvent) {
      $(document)
        .off('intel.xdk.services.' + dataEvent)
        .on('intel.xdk.services.' + dataEvent, function(evt, data) {
          Gauges.parseData(dataRpath, data, function(gatheredData) {
            var gage = gaugeWidgets[gaugeOptions.id];
            var value = null;
            var title = null;
    ...
    return gaugeWidgets[gaugeOptions.id] = {
